...
gaborArray = cell(u,v);     %helps creating vectors inside matrix of size uXv;
fmax = 0.25;
gama = sqrt(2);             %these are taken to be the standard deviation of gaussian factor, as i presume
eta = sqrt(2);

for i = 1:u                 %loop for creating scaled filter kernels
    
    fu = fmax/((sqrt(2))^(i-1));
    alpha = fu/gama;
    beta = fu/eta;
    
    for j = 1:v             %loop for creating Filter Kernels, each having different orientation(theta)
        tetav = ((j-1)/v)*pi;   %dividing Theta in V equal parts, starting with j-1 = 0;
        gFilter = zeros(m,n);
