function featureVector = gaborFeatures(img, name, gaborArray,d1,d2)
...
featureVector = featureVector(9,:);


%% Show filtered images (Please comment this section if not needed!)

%how does absoluting the matrix gives a very different result.
% % Show real parts of Gabor-filtered images
% figure('NumberTitle','Off','Name', strcat('Real parts of ', name));
% 
% for j = 1:v        
%     subplot(2,v/2,j)    
%     imshow(real(gaborResult{3,j}),[]);
% end
% 
% % Show magnitudes of Gabor-filtered images
% figure('NumberTitle','Off','Name', strcat('Magnitudes of ', name));
% for j = 1:v        
%     subplot(2,v/2,j)    
%     imshow(abs(gaborResult{3,j}),[]);
% end
